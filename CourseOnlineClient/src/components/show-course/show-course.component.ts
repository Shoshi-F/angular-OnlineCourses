...
@Component({
  ...
  standalone: true,
  imports: [
    CommonModule, // הוסף את CommonModule כאן
    MatIconModule,
    MatButtonModule,
    RouterLinkActive,
    RouterLink,
  ...
  templateUrl: './show-course.component.html',
  styleUrls: ['./show-course.component.css'] // ודא שזה styleUrls ולא styleUrl
})
export class ShowCourseComponent {
  courseId: string = '';
  course!: Course;  // משתנה course שיכול להיות null או מסוג Course
  ...
  constructor(
    ...
    private lessonservies: LessonService,
  ...
  ngOnInit(): void {
    this.activatedRoute.paramMap.subscribe(params => {
      ...
      if (this.courseId) {
        this.getCourseDetails();
        this.loadLesson(); // קריאה לפונקציה אחרי שקיבלנו את ה-ID
      }
    });
  }
  ...
  editLesson(Lesson: Lesson): void {
    const lesson: Lesson = this.lessons.find(c => c.id === Lesson.id) || {
      title: '',
      content: '',
      courseId: '',
      id: ''
